...
router.post('/register', async (req, res) => {
  try {
    ...
    if (existingUser) {
      return res.status(400).json({ error: 'Un compte avec cet email existe déjà. Essayez de vous connecter.' });
    }
    ...
    if (!organizationId) {
      ...
      if (!org) {
        return res.status(500).json({ error: 'Erreur de configuration. Contactez l\'administrateur.' });
      }
...
router.post('/login', async (req, res) => {
  try {
    ...
    if (!user.password) {
      return res.status(401).json({ error: 'Ce compte utilise une méthode de connexion différente.' });
    }
